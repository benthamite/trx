...
The development repository is on [[https://github.com/benthamite/trx][GitHub]].
...
** Download paths
:PROPERTIES:
:CUSTOM_ID: h:download-paths
:END:

#+vindex: trx-incomplete-dir
...
#+vindex: trx-category-directories
The user option ~trx-category-directories~ maps source categories to download directories.  See [[#h:moving-torrents][Moving torrents]] for the format and an example.
...
#+findex: trx-reclassify
#+vindex: trx-category-directories
The command ~trx-reclassify~ moves selected torrents to a directory chosen from ~trx-category-directories~.  The user option ~trx-category-directories~ is an alist whose keys are either regexes matched (case-insensitively) against a torrent's source category, or the symbol ~t~ as a fallback when no regex matches; values are absolute directory paths.  When adding torrents from a Jackett results buffer (see [[#h:jackett][Jackett search]]), ~trx-jackett-add~ consults the same option to choose the download directory based on the result's ~CategoryDesc~.  Use ~trx-reclassify~ to override or correct an automatic classification: it prompts via completion over the configured keys (with each entry's directory shown as an annotation) and moves the torrent server-side via ~torrent-set-location~.
...
The optional library =trx-jackett.el= integrates with [[https://github.com/Jackett/Jackett][Jackett]], a proxy server that provides a unified search API across many torrent indexers.  It requires a running Jackett instance.

** Configuration
:PROPERTIES:
:CUSTOM_ID: h:jackett-configuration
:END:

#+vindex: trx-jackett-host
#+vindex: trx-jackett-port
#+vindex: trx-jackett-api-key
#+vindex: trx-jackett-use-tls
#+vindex: trx-jackett-categories
The user option ~trx-jackett-host~ and ~trx-jackett-port~ specify the Jackett server address (default: ~localhost:9117~).

The user option ~trx-jackett-api-key~ is the API key for authentication.  If nil, it is resolved automatically in this order:

1. Jackett's own =ServerConfig.json= (checked in =~/Library/Application Support/Jackett/=, =~/.config/Jackett/=, and =/var/lib/jackett/=).
2. ~auth-source-search~ using the host and port (e.g. from =~/.authinfo.gpg=).

In most cases no configuration is needed---the key is read directly from Jackett's config file.

The user option ~trx-jackett-categories~ is a list of Torznab category IDs to filter results.  Common categories: 2000 (Movies), 3000 (Audio), 5000 (TV), 7000 (Books).  When nil, all categories are searched.

The user option ~trx-jackett-use-tls~, when non-nil, uses HTTPS.

** Searching
:PROPERTIES:
:CUSTOM_ID: h:jackett-searching
:END:

#+findex: trx-jackett-search
The command ~trx-jackett-search~ prompts for a query, searches all configured Jackett indexers, and displays results in a ~trx-jackett-results-mode~ buffer sorted by seeders.  Columns show seeders, leechers, size, age, tracker name, category, and title.

** Working with results
:PROPERTIES:
:CUSTOM_ID: h:jackett-results
:END:

#+findex: trx-jackett-add
#+findex: trx-jackett-browse-details
#+findex: trx-jackett-copy-magnet
#+findex: trx-jackett-search-again
In the results buffer:

- ~RET~ (~trx-jackett-add~) adds the torrent at point to Transmission via its magnet link or download URL.  When the result's ~CategoryDesc~ matches an entry in ~trx-category-directories~ (see [[#h:moving-torrents][Moving torrents]]), the torrent is added to the configured directory; otherwise it goes to the daemon's default download dir.
- ~b~ (~trx-jackett-browse-details~) opens the torrent's details page in a web browser.
- ~c~ (~trx-jackett-copy-magnet~) copies the magnet link to the kill ring.
- ~s~ (~trx-jackett-search-again~) runs a new search.

Columns are sortable by clicking the header or pressing ~S~.
