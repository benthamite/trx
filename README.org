...
- *Torrent list* (~trx-mode~): the main entry point.  Shows all torrents with their status, progress, speeds, and ratio.  From here you can add, start, stop, remove, verify, and label torrents, set speed and ratio limits, and navigate to per-torrent detail views.
...
The command ~trx-set-torrent-download~ sets the download speed limit for selected torrents.  When a single torrent is selected, the prompt shows its current limit and whether it is enabled.  A negative value disables the per-torrent limit.
